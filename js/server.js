...
var Server = function (config, callback) {
    ...
    app.use(function (req, res, next) {
        var result = ipfilter(config.ipWhitelist, { mode: config.ipWhitelist.length === 0 ? "deny" : "allow", log: false })(req, res, function (err) {
            ...
            res.status(403).send("This device is not allowed to access your device. <br> Please check your config.js or config.js.sample to change this.");
        });
    });
    app.use(helmet());
